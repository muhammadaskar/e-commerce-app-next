...
interface StockTransferReceived {
...
const StockTransferReceived = () => {
    const [stocksTransferReceived, setStocksTransferReceived] = useState<StockTransferReceived[]>([]);

    useEffect(() => {
        const fetchStockTransferReceived = async () => {
            const warehouseId = localStorage.getItem('warehouseId');
            const authToken = localStorage.getItem('authToken');
            if (warehouseId && authToken) {
                fetch(`${process.env.NEXT_PUBLIC_WAREHOUSE_SERVICE}/stock-transfers/by-destination-warehouse/${warehouseId}`, {
                    headers: {
                    ...
                    .catch((error) => {
                        console.error('Error fetching StockTransferReceived:', error);
                    });
            } else {
            }
        };

        fetchStockTransferReceived();
    }, []);
    ...
    return (
        <div className="flex h-screen">
            ...
            <div className="flex-1 p-6 bg-gray-100">
                ...
                <p className="mt-4 text-lg text-gray-600">List Stock Transfer Received</p>
                        ...
                        <tbody>
                            {stocksTransferReceived.map((order) => (
                                <tr key={order.stockTransferId} className="border-b border-gray-300">
                                    ...
                                    <td className="px-6 py-4 text-black">{order.sourceWarehouse.name}</td>
                                    ...
                                    {/* <td>
                                        <Link
                                            href={`/dashboard/StockTransferReceived/${order.stockTransferId}`}
...
export default StockTransferReceived;
